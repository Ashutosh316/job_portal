...
import { App_API_END_POINT, JOB_API_END_POINT } from '@/utils/api';
...
const JobDescription = () => {
    ...
    const applyJobHandler = async () =>{
        try {
            ...
            if(res.data.success){
                setIsApplied(true) // update now user has applied
                const updatedSingleJob = {...singleJob, applications:[...singleJob.applications,{applicant:user?._id}]}  //to the end of the array. This represents the current user who has just applied for the job.
    ...
    useEffect(()=>{
        ...
        fetchSingleJob();
    },[jobId , dispatch , user?.id])   //new jobid load hogi jab jab multiple job aayegi or different user jab aayega to uski bhi id load hogi
    return (
